...
const out = await page.evaluate(async () => {
  ...
  const probes = {
    ...
    // ── core read channels behind each screen ──
    'dashboard.stats': () => e.dashboard.stats(),
    'articles.list': () => e.articles.list({}),
    'articles.count': () => e.articles.count({}),
    'articles.categories': () => e.articles.categories(),
    'sources.list': () => e.sources.list(),
    'publish.platforms': () => e.publish.platforms(),
    'social.platforms': () => e.social.platforms(),
    'social.connections': () => e.social.connections(),
    'social.configuredPlatforms': () => e.social.configuredPlatforms(),
    'templates.articleList': () => e.templates.articleList(),
    'templates.publishList': () => e.templates.publishList(),
    'trendRadar.list': () => e.trendRadar.list(),
    'trendRadar.stats': () => e.trendRadar.stats(),
    'ai.status': () => e.ai.status(),
    'ai.listModels': () => e.ai.listModels(),
    'media.list': () => e.media.list(),
    'media.stats': () => e.media.stats(),
    'tasks.list': () => e.tasks.list(),
    'analytics.alerts': () => e.analytics.alerts(),
    'pipeline.kanban': () => e.pipeline.kanban(),
    'pipeline.config': () => e.pipeline.config(),
    'autopilot.status': () => e.autopilot.status(),
    'autopilot.config': () => e.autopilot.config(),
    'users.list': () => e.users.list(),
    'backup.list': () => e.backup.list(),
    'tenants.list': () => e.tenants.list(),
    'updater.status': () => e.updater.status(),
    'stealth.status': () => e.stealth.status(),
    'api.status': () => e.api.status(),
    'pin.status': () => e.pin.status(),
    'biometric.status': () => e.biometric.status(),
  };
  const res = {};
